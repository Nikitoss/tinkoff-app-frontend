...
const MemberCard = ({ username, userId, accetionDate, projectId }: { username: string, userId: number, accetionDate: number, projectId: number }) => (
    ...
    return (
        <>
            {members.map(({ userId, username, accetionDate }) => (
                <MemberCard userId={userId!} username={username!} accetionDate={accetionDate!} key={userId} projectId={projectId} />
            ))}
        </>
    )
}
...
export default function Page() {
    ...
    useEffect(() => {
        Promise.all([getProjectById(projectId), getProjectSettings(projectId)]).then(([{ data: project, error: projectError }, { data: settings, error: settingsError }]) => {
            setProject(project)
            if (settings) {
            ...
            >
                <RoleChecker>
                    <SettingsForm
                        ...
                        initialVoteCount={Number(settings.voteCount || 1)}
                        initialPeriod={Number(settings.period || 1)}
                        onSubmit={onSubmit}
                    />
                </RoleChecker>
            </Grid>
