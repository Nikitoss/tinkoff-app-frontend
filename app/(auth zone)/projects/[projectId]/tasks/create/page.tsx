    ...
    return (
        <main>
                ...
                >
                    <div className="w-3/5 py-4 rounded-lg bg-neutral-200 space-y-2 inset-x-0 mb-12">
                        ...
                        <div className="flex justify-center mt-4 px-36">
                            <Link className="w-full h-12 px-10 mt-2 border flex justify-center gap-2 rounded-lg bg-yellow-300 hover:shadow hover:bg-gray-200 transition duration-300" href="/projects/3">
            ...
            <div className="pt-4 mr-5 grid grid-cols-4 gap-5 relative">
                <div className="col-start-2 col-span-2 w-full mt-36 px-10 py-4 rounded-lg bg-neutral-200 space-y-2 inset-x-0">
                    <span className="font-bold text-2xl flex justify-center">Добавить задачу</span>
                    ...
                    <div className="space-y-1">
                        <label htmlFor="name">Опишите её (желательно, чтобы все её поняли)</label>
                        <input type="text" id="name" placeholder="Как ни подойдешь на кухню, все время нет печенек, я начинаю грустить и становлюсь злюкой." className="w-full flex justify-center rounded-[7px] px-1" />
                    </div> 
                    <div className="flex justify-center mt-4 px-36">
...
async function getData(projectId: number) {
    const res = await fetch(`${process.env.SERVER_URL}/api/v1/projects/${projectId}`)
